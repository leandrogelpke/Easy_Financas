...
| 21/08 | **Lote final da auditoria — zero hardcode**: aba Projeção 100% calculada (`compute_proj_cfg` → `@@PROJ_CFG@@`: receita = média 6m fechados ×80/100/120%, despesa op = média 3m fechados sem aporte/estrutural, estruturais = parcelas reais do em aberto, caixa = Bling, horizonte rolante); Pipeline extraído pra `pipeline_data.json` (contagens calculadas no JS); regime de caixa vira regra `data_caixa()` (data_pagamento quando existir, senão vencimento — migra sozinho); **`fornecedores_classificacao.json` = fonte única** de classificação (build-html + dre_render; literais in-code viraram fallback); clientes sem override contados como "auto" no subtítulo; `LIMIARES` nomeados; gate novo `test_js_sintaxe_valida` (node --check) no test_build; `weekly.sh` bloqueado sem `EF_FORCE_LOCAL=1` (rotação de token local quebra o Secret do CI) | build-html · dre_render · template · test_build · weekly.sh |
